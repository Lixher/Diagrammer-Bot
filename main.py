# main.py (ФИНАЛЬНАЯ МНОГОЯЗЫЧНАЯ ВЕРСИЯ)
...
import config, database, nodes, edges, renderer, admin, localization

# Настройка логирования
logging.basicConfig(format="%(asctime)s - %(name)s - %(levelname)s - %(message)s", level=logging.INFO)

# --- Состояния диалога ---
(SELECT_LANGUAGE, MAIN_MENU, ADD_NODE, ADD_EDGE_START, ADD_EDGE_END, MANAGE_NODES,
 EDIT_NODE_TEXT, SETTINGS, MANAGE_EDGES, DIAGRAM_MENU, SAVE_DIAGRAM_NAME, LOAD_DIAGRAM, CHANGE_LANGUAGE) = range(13)

# --- ГЛАВНАЯ ФУНКЦИЯ ЛОКАЛИЗАЦИИ ---
def _(user_id: int, key: str) -> str:
    """Возвращает строку на языке пользователя."""
    user_data = database.get_user_data(user_id)
    lang = user_data.get('settings', {}).get('language', 'ru')
    return localization.locales.get(lang, localization.locales['ru']).get(key, localization.locales['en'].get(key, f"_{key}_"))
...
async def update_diagram_view(update: Update, context: ContextTypes.DEFAULT_TYPE, message: str = ""):
    ...
    keyboard = [
        [InlineKeyboardButton(_(user_id, 'btn_add_node'), callback_data="add_node"), InlineKeyboardButton(_(user_id, 'btn_manage_nodes'), callback_data="manage_nodes")],
        [InlineKeyboardButton(_(user_id, 'btn_add_edge'), callback_data="add_edge"), InlineKeyboardButton(_(user_id, 'btn_manage_edges'), callback_data="manage_edges")],
        [InlineKeyboardButton(_(user_id, 'btn_diagrams'), callback_data="diagram_menu"), InlineKeyboardButton(_(user_id, 'btn_settings'), callback_data="settings")],
        [InlineKeyboardButton(_(user_id, 'btn_download_png'), callback_data="generate_file")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    caption_prefix = _(user_id, 'editing_diagram').format(name=active_diagram_name, message=message)
    
    if not diagram.get('edges'):
        final_text = caption_prefix
        if not diagram.get('nodes'):
            final_text += _(user_id, 'diagram_is_empty')
        else:
            final_text += _(user_id, 'current_nodes')
            for node in diagram['nodes']:
                node_repr = f"«{node['content']}»" if node['type'] == 'text' else _(user_id, 'node_repr_image')
                final_text += f"- {node_repr}\n"
            final_text += _(user_id, 'add_edge_to_see_preview')
    ...
    if not image_path or os.path.getsize(image_path) == 0:
        await context.bot.send_message(chat_id=user_id, text=_(user_id, 'error_critical_render'))
        if image_path: os.remove(image_path)
        return MAIN_MENU

    caption_text = caption_prefix + _(user_id, 'stats_line').format(nodes=len(diagram['nodes']), edges=len(diagram['edges']))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Предлагаем выбор языка, даже если пользователь уже есть
    keyboard = [
        [InlineKeyboardButton("Русский 🇷🇺", callback_data="set_lang_ru")],
        [InlineKeyboardButton("English 🇬🇧", callback_data="set_lang_en")],
    ]
    await update.message.reply_text(
        localization.locales['ru']['select_language'],
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def set_language(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_data = database.get_user_data(user_id)
    user_data['settings']['language'] = lang_code
    database.save_user_data(user_id, user_data)
    
    await query.edit_message_text(
        _(user_id, 'welcome').format(mention=update.effective_user.mention_html()),
        parse_mode='HTML'
    )
    return ADD_NODE
...
async def add_node_text_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    database.save_user_data(user_id, user_data)
    await update_diagram_view(update, context, message=_(user_id, 'node_added_text').format(text=update.message.text))
    return MAIN_MENU

async def add_node_photo_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user_id = update.effective_user.id
    processing_message = await context.bot.send_message(chat_id=user_id, text=_(user_id, 'saving_image'))
    ...
    await update_diagram_view(update, context, message=_(user_id, 'node_added_image'))
    return MAIN_MENU
...
async def add_node_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_id = query.from_user.id
    await query.message.delete()
    await context.bot.send_message(chat_id=user_id, text=_(user_id, 'prompt_add_node'))
    return ADD_NODE
...
async def manage_nodes_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not diagram['nodes']:
        await query.answer(_(user_id, 'error_no_nodes_to_manage'), show_alert=True)
        return MAIN_MENU
    keyboard = []
    for node in diagram['nodes']:
        if node['type'] == 'text':
            ...
            label_preview = _(user_id, 'node_repr_image')
    ...
    keyboard.append([InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="back_to_main")])
    await edit_menu(query, _(user_id, 'menu_manage_nodes'), InlineKeyboardMarkup(keyboard))
    return MANAGE_NODES
...
async def delete_node_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    database.save_user_data(user_id, user_data)
    await update_diagram_view(update, context, message=_(user_id, 'node_deleted'))
    return MAIN_MENU
...
async def edit_node_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.delete()
    await context.bot.send_message(chat_id=user_id, text=_(user_id, 'prompt_edit_node'))
...
async def edit_node_text_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    database.save_user_data(user_id, user_data)
    await update_diagram_view(update, context, message=_(user_id, 'node_edited'))
    return MAIN_MENU
...
async def add_edge_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if len(diagram['nodes']) < 2:
        await query.answer(_(user_id, 'error_need_2_nodes'), show_alert=True)
        return MAIN_MENU
    keyboard = []
    for node in diagram['nodes']:
        label = node['content'] if node['type'] == 'text' else _(user_id, 'node_repr_image')
        keyboard.append([InlineKeyboardButton(label, callback_data=f"edge_start_{node['id']}")])
    keyboard.append([InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="back_to_main")])
    await edit_menu(query, _(user_id, 'prompt_add_edge_start'), InlineKeyboardMarkup(keyboard))
...
async def add_edge_select_start_node(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard.append([InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="back_to_main")])
    await edit_menu(query, _(user_id, 'prompt_add_edge_end'), InlineKeyboardMarkup(keyboard))
    return ADD_EDGE_END
...
async def add_edge_select_end_node(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    database.save_user_data(user_id, user_data)
    await update_diagram_view(update, context, message=_(user_id, 'edge_added'))
    return MAIN_MENU
...
async def manage_edges_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not diagram['edges']:
        await query.answer(_(user_id, 'error_no_edges_to_manage'), show_alert=True)
        return MAIN_MENU
    nodes_map = {node['id']: (node['content'] if node['type'] == 'text' else _(user_id, 'node_repr_image')) for node in diagram['nodes']}
    keyboard = []
    ...
    keyboard.append([InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="back_to_main")])
    await edit_menu(query, _(user_id, 'menu_manage_edges'), InlineKeyboardMarkup(keyboard))
    return MANAGE_EDGES
...
async def delete_edge_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    database.save_user_data(user_id, user_data)
    await update_diagram_view(update, context, message=_(user_id, 'edge_deleted'))
    return MAIN_MENU
...
async def settings_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    theme = user_data['settings'].get('theme', 'dark')
    keyboard = [
        [InlineKeyboardButton(f"{'✅' if theme == 'dark' else ''} " + _(user_id, 'btn_dark_theme'), callback_data="set_theme_dark")],
        [InlineKeyboardButton(f"{'✅' if theme == 'light' else ''} " + _(user_id, 'btn_light_theme'), callback_data="set_theme_light")],
        [InlineKeyboardButton(_(user_id, 'btn_lang'), callback_data="change_lang_prompt")],
        [InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="back_to_main")]]
    await edit_menu(query, _(user_id, 'menu_settings'), InlineKeyboardMarkup(keyboard))
    return SETTINGS
...
async def set_theme_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    database.save_user_data(user_id, user_data)
    await update_diagram_view(update, context, message=_(user_id, 'theme_changed'))
    return MAIN_MENU

async def change_lang_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer()
    user_id = query.from_user.id
    keyboard = [
        [InlineKeyboardButton("Русский 🇷🇺", callback_data="set_lang_ru")],
        [InlineKeyboardButton("English 🇬🇧", callback_data="set_lang_en")],
        [InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="settings_back")]
    ]
    await edit_menu(query, _(user_id, 'menu_lang'), InlineKeyboardMarkup(keyboard))
    return CHANGE_LANGUAGE

async def generate_file_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; user_id = query.from_user.id
    await query.answer(_(user_id, 'preparing_file'), show_alert=False)
    user_data = database.get_user_data(user_id)
    diagram = user_data["diagrams"][user_data["active_diagram_name"]]
    theme = user_data['settings'].get('theme', 'dark')
    image_path = renderer.render_diagram(diagram, user_id, theme)
    if image_path and os.path.getsize(image_path) > 0:
        ...
        await query.answer(_(user_id, 'error_nothing_to_download'), show_alert=True)

async def diagram_menu_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer()
    user_id = query.from_user.id
    keyboard = [
        [InlineKeyboardButton(_(user_id, 'btn_save_current'), callback_data="save_diagram")],
        [InlineKeyboardButton(_(user_id, 'btn_load'), callback_data="load_diagram_prompt")],
        [InlineKeyboardButton(_(user_id, 'btn_create_new'), callback_data="new_diagram")],
        [InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="back_to_main")]]
    await edit_menu(query, _(user_id, 'menu_diagrams'), InlineKeyboardMarkup(keyboard))
    return DIAGRAM_MENU
...
async def save_diagram_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_id = query.from_user.id
    await query.message.delete()
    await context.bot.send_message(chat_id=user_id, text=_(user_id, 'prompt_save_diagram'))
...
async def save_diagram_name_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if database.save_active_diagram(user_id, new_name):
        await update_diagram_view(update, context, message=_(user_id, 'diagram_saved').format(name=new_name))
    else:
        await update_diagram_view(update, context, message=_(user_id, 'error_diagram_exists'))
    return MAIN_MENU

async def load_diagram_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer()
    user_id = query.from_user.id
    saved_diagrams = database.list_saved_diagrams(user_id)
    if not saved_diagrams:
        await query.answer(_(user_id, 'error_no_diagrams_saved'), show_alert=True)
        return DIAGRAM_MENU
    keyboard = [[InlineKeyboardButton(name, callback_data=f"load_{name}")] for name in saved_diagrams]
    keyboard.append([InlineKeyboardButton(_(user_id, 'btn_back'), callback_data="diagram_menu_back")])
    await edit_menu(query, _(user_id, 'prompt_load_diagram'), InlineKeyboardMarkup(keyboard))
    return LOAD_DIAGRAM
...
async def load_diagram_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if database.set_active_diagram(user_id, diagram_name):
        await update_diagram_view(update, context, message=_(user_id, 'diagram_loaded').format(name=diagram_name))
    return MAIN_MENU

async def new_diagram_action_from_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer()
    user_id = query.from_user.id
    new_name = database.create_new_diagram(user_id)
    await update_diagram_view(update, context, message=_(user_id, 'diagram_created').format(name=new_name))
    return MAIN_MENU
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            SELECT_LANGUAGE: [CallbackQueryHandler(set_language, pattern="^set_lang_")],
            MAIN_MENU: [
            ...
            SETTINGS: [
                CallbackQueryHandler(set_theme_action, pattern="^set_theme_"),
                CallbackQueryHandler(change_lang_prompt, pattern="^change_lang_prompt$"),
                CallbackQueryHandler(back_to_main_menu, pattern="^back_to_main$")
            ],
            CHANGE_LANGUAGE: [
                CallbackQueryHandler(set_language, pattern="^set_lang_"),
                CallbackQueryHandler(settings_prompt, pattern="^settings_back$")
            ],
            DIAGRAM_MENU: [
    ...
    print("Бот запущен с поддержкой локализации.")
